...
from app_modules.colors import ENERGY_TYPE_COLORS  # Importing custom color mappings
...
def create_pie_chart(df):
    ...
    tech_volume["percent"] = (tech_volume["total_volume_sold"] / tech_volume["total_volume_sold"].sum()) * 100
...
def create_bar_chart(df, time_interval):
    ...
    grouped_df = df.groupby(["date", "energy_type"])["total_volume_sold"].sum().reset_index()

    if time_interval == 'Yearly':
        grouped_df['year'] = grouped_df['date'].dt.year
        grouped_df = grouped_df.groupby(['year', 'energy_type'], as_index=False)['total_volume_sold'].sum()
        x_col = 'year'
    else:
        x_col = 'date'
        
    bar_fig = px.bar(grouped_df, x=x_col, y="total_volume_sold", color="energy_type", color_discrete_map=ENERGY_TYPE_COLORS)
    return bar_fig
...
def display_combined_chart(df, region, width, height, time_interval):
    ...
    fig = make_subplots(rows=1, cols=2, column_widths=[0.5, 0.5], subplot_titles=("Proportion of Volume by Energy Type", "Total Volume per Energy Type over Time"), specs=[[{"type": "domain"}, {"type": "xy"}]])
    for annotation in fig["layout"]["annotations"]:
        annotation["font"] = dict(size=20)

    # Adding traces for pie and bar charts to the subplot
    fig.add_trace(go.Pie(labels=pie_fig["data"][0]["labels"], values=pie_fig["data"][0]["values"], textinfo="percent+label", name="", domain={"column": 0}, showlegend=False, marker_colors=[ENERGY_TYPE_COLORS[label] for label in pie_fig["data"][0]["labels"]]), row=1, col=1)
    for trace in bar_fig["data"]:
        fig.add_trace(trace.update(showlegend=True), row=1, col=2)

    # Updating layout and displaying the combined chart
    fig.update_layout(legend_traceorder="reversed", height=height, width=width, barmode="stack", title_text=f"{region}", title_x=0.3, title_font=dict(size=SUPTITLE_FONT_SIZE), legend_title_text="Energy Types :", legend_font_size=22, legend_title_font_size=18)
...
def create_energy_bar_chart(df, energy_type, time_interval):
    ...
    fig = px.bar(grouped_df, x="date", y="total_volume_sold", labels={"total_volume_sold": "Total Volume Sold"}, title=f'Total Volume Sold Over Time for {energy_type} Energy')
...
def create_energy_season_bar_chart(df, energy_type):
    ...
    seasons_mapping = {12: "Winter", 1: "Winter", 2: "Winter", 3: "Spring", 4: "Spring", 5: "Spring", 6: "Summer", 7: "Summer", 8: "Summer", 9: "Autumn", 10: "Autumn", 11: "Autumn"}
    df["season"] = df["date"].dt.month.map(seasons_mapping)
    grouped_df = df.groupby("season")["total_volume_sold"].sum().reset_index()
    grouped_df["percentage_of_total"] = (grouped_df["total_volume_sold"] / df["total_volume_sold"].sum()) * 100
    grouped_df["season"] = grouped_df["season"].astype(pd.CategoricalDtype(categories=["Winter", "Spring", "Summer", "Autumn"], ordered=True))
    grouped_df = grouped_df.sort_values(by="season")
    
    fig = px.bar(grouped_df, x="season", y="percentage_of_total", color="season", color_discrete_map={"Spring": "green", "Summer": "yellow", "Winter": "blue", "Autumn": "orange"}, labels={"percentage_of_total": "Percentage of Sold Energy", "season": "Season"}, title=f"Percentage of {energy_type} Sold per Season")
    return fig
...
def display_combined_energy_chart(df, region, energy_type, time_interval):
    ...
    fig = make_subplots(rows=1, cols=2, column_widths=[0.5, 0.5], subplot_titles=(f"Total Volume Sold Over Time for {energy_type} Energy", f"Percentage of {energy_type} Sold per Season"))
    ...
    fig.update_layout(title_text=region, title_x=0.4, title_font=dict(size=SUPTITLE_FONT_SIZE))
    st.plotly_chart(fig, use_container_width=True)
...
def create_energy_region_pie_chart(region_df, energy_type, n):
    ...
    top_n_df = region_df.head(n)
    if len(region_df) > n:
        other_df = pd.DataFrame({'region': ['Other regions'], col_name: [region_df.iloc[n:][col_name].sum()]})
        final_df = pd.concat([top_n_df, other_df], ignore_index=True)
    else:
        final_df = top_n_df

    fig = px.pie(final_df, names='region', values=col_name, title=f'{energy_type} Total Volume Sold by Region')
